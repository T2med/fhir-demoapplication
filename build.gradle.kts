...
tasks.register<Exec>("packageApp") {
    dependsOn("jar")
    group = "distribution"
    description = "Erstellt ein macOS App-Image mit jpackage."

    workingDir = projectDir
    
    doFirst {
        val outDir = file("out/$appName.app")
        if (outDir.exists()) {
            println("Lösche altes App-Image: $outDir")
...
tasks.register<Exec>("installApp") {
    ...
    group = "distribution"
    description = "Installiert die App in den /Applications Ordner."
